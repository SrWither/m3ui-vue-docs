...
import { ref } from 'vue'
import {
  MBottomSheet, MSideSheet, MLoadingOverlay, MWindow, MBar, MChatBubble,
...
const barMacMaximized = ref(false)
...
#actions  — Extra buttons in the title bar (before minimize/close)</code></pre>
    ...
    <!-- ── MBar ─────────────────────────────────────────────────────────── -->
    <h2 id="mbar" class="mb-4 mt-14 text-headline-small font-medium">MBar</h2>
    <p class="mb-6 max-w-3xl text-body-medium text-on-surface-variant">
...
&lt;/MBar&gt;`"
    ...
    <MCard class="mt-4 border-l-4 border-l-tertiary p-5">
      <p class="mb-2 text-title-small font-medium">Slots &amp; events</p>
      <pre class="overflow-x-auto rounded-lg bg-surface-container p-3 text-body-small"><code>#icon      — Replaces the icon (e.g. an &lt;img&gt; logo instead of a Material Symbol)
#title     — Replaces the title text
#default   — Extra content between the title and the trailing area (drag-adjacent, e.g. tabs)
#trailing  — Action buttons right before minimize/maximize/close

@minimize / @maximize / @close — forward these to your preload-exposed
  Electron IPC calls (e.g. window.api.minimize()); MBar never touches
  the real window itself.</code></pre>
    </MCard>
  </div>
</template>
