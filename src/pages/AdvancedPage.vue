...
const swirl = \`${swirlGlsl.replace(/`/g, '\\`')}\``"
    ...
    <!-- ── MQRCode ──────────────────────────────────────────────────────── -->
    <h2 id="mqrcode" class="mb-4 mt-14 text-headline-small font-medium">MQRCode</h2>

    <ComponentDemo
      title="QR code generator"
      description="Renders as inline SVG — crisp at any size, no canvas or image assets. Available from the /qrcode entry point with qrcode-generator as an optional peer dependency, so it never adds weight to the main bundle."
