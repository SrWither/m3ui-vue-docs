...
const versions: Version[] = [
  {
    ...
    changes: {
      fixed: [
        ...
        'MMenu and MContextMenu\'s open/close animation read as stiff opening and janky closing — the scale motion was too subtle (a 5% bump) and, worse, closing used a shorter, differently-shaped animation than opening instead of mirroring it. Both now use a proper 80%-to-100% scale over a matching duration in both directions, so closing actually looks like the reverse of opening instead of an abrupt cut',
        ...
        'MCarousel left visible dead space between items while scrolling — a neighboring item\'s reserved width stayed at its full size even as its visible mask shrank, instead of the layout closing the gap as it shrank. Fixed so items reflow smoothly with no dead space',
      ],
      changed: [
  ...
  },
  {
    ...
    changes: {
      added: [
        'MCodeEditor gained C, C++, Rust, and Assembly language support — C and C++ share the same parser (there\'s no separate C-only package), and Assembly highlighting is more basic than the others since it has no dedicated modern CodeMirror package',
